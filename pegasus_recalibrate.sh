            ...
            if [ $arrayCount -gt 0 ] ; then
                ...
                if [ $skipGroup -ne 1 ] ; then
                    ...
                    do
                        ...
                        sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,GATKPATH=$gatkPath,KNOWN=$known,RECALBAM=$rcBamFile,D=$d,REF=$ref,BAMFILE=$inputForNext,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_recalibrate.sh
                        if [ $? -eq 0 ] ; then
