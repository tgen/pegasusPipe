			...
			if [ $arrayCount -gt 0 ] ; then
				...
				if [ $missingBamSample -eq 0 ] ; then
					...
					if [[ -e $mergedBamName.mergeBamPass || -e $mergedBamName.mergeBamInQueue || -e $mergedBamName.mergeBamFail ]] ; then
						...
						qsub -A $debit -l nodes=1:ppn=$nCores -v PICARDPATH=$picardPath,SAMTOOLSPATH=$samtoolsPath,CNT=$bamCount,RUNDIR=$runDir,NXT1=$nxtStep1,BAMLIST="$bamList",MERGEDBAM=$mergedBamName,D=$d $pbsHome/pegasus_mergeBams.pbs
						if [ $? -eq 0 ] ; then
