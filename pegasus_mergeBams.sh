			...
			if [ $arrayCount -gt 0 ] ; then
				...
				if [ $missingBamSample -eq 0 ] ; then
					...
					if [[ -e $mergedBamName.mergeBamPass || -e $mergedBamName.mergeBamInQueue || -e $mergedBamName.mergeBamFail ]] ; then
						...
						sbatch -n 1 -N 1 --cpus-per-task $nCores --export PICARDPATH=$picardPath,SAMTOOLSPATH=$samtoolsPath,CNT=$bamCount,RUNDIR=$runDir,NXT1=$nxtStep1,BAMLIST="$bamList",MERGEDBAM=$mergedBamName,D=$d $pegasusPbsHome/pegasus_mergeBams.pbs
						if [ $? -eq 0 ] ; then
