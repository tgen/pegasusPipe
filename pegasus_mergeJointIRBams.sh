	...
	if [ $bigBamDetected -eq 0 ] ; then
		...
		if [ $missingGrp -eq 0 ] ; then
			...
			do
				...
				sbatch -n 1 -N 1 --cpus-per-task $nCores --export NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,NXT4=$nxtStep4,NXT5=$nxtStep5,NEWLOC=$newLoc,PICARDPATH=$picardPath,SAMTOOLSPATH=$samtoolsPath,CNT=25,MERGEDBAM=$mergedBam,BAMLIST="$fileList",RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_mergeBamsForBigJIR.pbs
				if [ $? -eq 0 ] ; then
