...
do
	...
	tophat) echo "tophat case"
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export CUFFDIFF2VCFPATH=$cuffdiff2vcfPath,PROCESSCDLISTPATH=$processcdlistPath,CUFFDIFFPATH=$cuffdiffPath,RUNDIR=$runDir,DIRNAME=$cdDir,GTF=$gtf,BAM1="'"$normlList"'",BAM2="'"$tumorList"'",REF=$ref,MASK=$gtfmask,REF=$ref,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_cuffDiff.sh
		if [ $? -eq 0 ] ; then
	...
	star) echo "star case"
		...
		if [ $rnaTumorStrand == "FIRST" ] ; then
			...
			sbatch -n 1 -N 1 --cpus-per-task $nCores --export CUFFDIFF2VCFPATH=$cuffdiff2vcfPath,PROCESSCDLISTPATH=$processcdlistPath,CUFFDIFFPATH=$cuffdiffPath,RUNDIR=$runDir,DIRNAME=$cdDir,GTF=$gtf,BAM1="'"$normlList"'",BAM2="'"$tumorList"'",REF=$ref,MASK=$gtfmask,REF=$ref,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_firstStrandedCuffDiff.sh
			if [ $? -eq 0 ] ; then
				touch $cdDir.cuffDiffInQueue
			else
				((qsubFails++))
			...
			sleep 2
                        ...
                        sbatch -n 1 -N 1 --cpus-per-task $nCores --export CUFFDIFF2VCFPATH=$cuffdiff2vcfPath,PROCESSCDLISTPATH=$processcdlistPath,CUFFDIFFPATH=$cuffdiffPath,RUNDIR=$runDir,DIRNAME=$cdDir,GTF=$gtf,BAM1="'"$normlList"'",BAM2="'"$tumorList"'",REF=$ref,MASK=$gtfmask,REF=$ref,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_secondStrandedCuffDiff.sh
                        if [ $? -eq 0 ] ; then
                                touch $cdDir.cuffDiffInQueue
                        else
                                ((qsubFails++))
