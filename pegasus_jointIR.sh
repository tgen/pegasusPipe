...
do
	...
	#		qsub -A $debit -l nodes=1:ppn=$nCores -v GRPNAME=$grpName,CHRGRP=$chrGrp,WORKDIR=$workDir,GATKPATH=$gatkPath,TRK=$trackName,INTS=$irIntFile,INDELS=$indels,DIRNAME=$jirDir,BAMLIST="'$sampleList'",REF=$ref,NXT1=$nxtStepA,RUNDIR=$runDir,D=$d $pbsHome/pegasus_jointIRsplit.pbs
	#		if [ $? -eq 0 ] ; then
