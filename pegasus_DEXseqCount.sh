	...
	do
		...
		if [ ! -e $rnaMarkDupPass ] ; then
			...
			if [[ -e $DEXseqOut.DEXseqCountPass || -e $DEXseqOut.DEXseqCountFail || -e $DEXseqOut.DEXseqCountInQueue ]] ; then
				...
				sbatch -n 1 -N 1 --cpus-per-task $nCores --export DEXSEQOUT=$DEXseqOut,DEXSEQCOUNTPATH=$DEXseqCountPath,DEXSEQGFF=$DEXseqGff,RNABAM=$rnaBam,RUNDIR=$runDir,DEXSEQOUTDIR=$DEXseqCountDir,DEXSEQCOUNTOUT=$DEXseqCountOut,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_DEXseqCount.sh
				if [ $? -eq 0 ] ; then
	...
	do
		...
		if [ ! -e $rnaMarkDupPass ] ; then
			...
			if [[ -e $DEXseqOut.DEXseqCountPass || -e $DEXseqOut.DEXseqCountFail || -e $DEXseqOut.DEXseqCountInQueue ]] ; then
				...
				echo "### Submitting $samName to queue for DEXseqCount..."
				sbatch -n 1 -N 1 --cpus-per-task $nCores --export DEXSEQOUT=$DEXseqOut,DEXSEQCOUNTPATH=$DEXseqCountPath,DEXSEQGFF=$DEXseqGff,RUNDIR=$runDir,RNABAM=$rnaBam,DEXSEQOUTDIR=$DEXseqCountDir,DEXSEQCOUNTOUT=$DEXseqCountOut,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_DEXseqCount.sh
