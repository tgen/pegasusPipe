...
do
	...
	*)	echo "### Something else"
	...
	sbatch -n 1 -N 1 --cpus-per-task $nCores --export ASSAY=$assay,STRELKAPATH=$strelkaPath,WD=$wd,REF=$ref,NXT1=$nxtStep1,NORMAL=$normalBamFile,TUMOR=$tumorBamFile,D=$d $pegasusPbsHome/pegasus_strelka.sh
	if [ $? -eq 0 ] ; then
