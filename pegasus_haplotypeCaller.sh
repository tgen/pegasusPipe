		...
		if [[ -e ${trackName}_Step${STEP}.hcInQueue || -e ${trackName}_Step${STEP}.hcPass || -e ${trackName}_Step${STEP}.hcFail ]] ; then
			...
			continue
                 ...
                 #       echo Starting Haplotype caller Step${STEP}
                  #      qsub -A $debit -l nodes=1:ppn=$nCores -v GATKPATH=$gatkPath,STEPCOUNT=$STEP_COUNT,TRK=$trackName,KNOWN=$snps,BAMLIST="'$sampleList'",TRK=$trackName,CHRLIST=$chrList,REF=$ref,STEP="chr${STEP}",NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_haplotypeCaller.pbs
                   #     if [ $? -eq 0 ] ; then
