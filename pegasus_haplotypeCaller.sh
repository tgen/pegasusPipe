...
do
	...
	do	
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --export GATKPATH=$gatkPath,STEPCOUNT=$STEP_COUNT,TRK=$trackName,KNOWN=$snps,BAMLIST="'$sampleList'",TRK=$trackName,CHRLIST=$chrList,REF=$ref,STEP=${STEP},NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_haplotypeCaller.sh
        if [ $? -eq 0 ] ; then
		...
		if [ $jrRequested -gt 0 ] ; then
			...
			if [[ ! -e $inBam.mdPass || ! -e $mdBam ]] ; then
				...
				do
					...
					sbatch -n 1 -N 1 --cpus-per-task $nCores --export GATKPATH=$gatkPath,STEPCOUNT=$STEP_COUNT,TRK=$mdBam,KNOWN=$snps,BAMLIST=$mdBam,CHRLIST=$chrList,REF=$ref,STEP=${STEP},NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_haplotypeCallerSingle.sh
					if [ $? -eq 0 ] ; then
