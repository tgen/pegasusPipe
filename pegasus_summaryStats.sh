...
if [ $alreadyDone -eq 0 ] ; then
	...
	sbatch -n 1 -N 1 --cpus-per-task $nCores --export SUMSTATSPATH=$sumStatsPath,RUNDIR=$runDir,EMAIL=$email,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_summaryStats.sh
	if [ $? -eq 0 ] ; then
