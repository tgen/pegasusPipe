...
VCFMERGER="/home/tgenref/binaries/vcfMerger/vcfMerger/pecan.merge.3vcfs.main.sh"
VCFMERGER_DIR="/home/tgenref/binaries/vcfMerger/vcfMerger/"
...
POST_MERGE_VENN="/home/tgenref/binaries/vcfMerger/vcfMerger/pecan.Venn_postMMRF_specific_filtering.sh"
...
do
    ...
    if [[ "$kitName" == "TSE61" ]] ; then
        ...
        bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/agilent_sureselect_v5_strexome/Strexome_targets_sortedTabs2_padded25.bed"
    elif [[ "$kitName" == *SCR ]] ; then
        bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/agilent_sureselect_cre_v1/Agilent_Clinical_Research_Exome_hs37d5_TargetsPadded25_sortedTabs2_Picard.txt"
    fi
    ...
    if [ ! -e $fbTrackName.freebayes_All.vcf.snpEffPass  ] ; then
        ...
        if [[ -e ${mergerDir}/${usableName}.vcfMergerPass || -e ${mergerDir}/${usableName}.vcfMergerInQueue || -e ${mergerDir}/${usableName}.vcfMergerFail ]] ; then
            ...
            continue
                ...
                sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,VTPATH=$vtPath,SNPEFFPATH=$snpeffPath,SNPSIFT=$snpSift,DBNSFP=$DBNSFP,DBNSP=$DBNSP,TRACKNAME=$trackName,SAMTOOLS=$samTools,VARSCAN=$varScan,REF=$ref,DICT=$refDict,COSMIC=$COSMIC,KG=$KG,NHLBI=$NHLBI,SNPS=$snps,INDELS=$indels,GATKPATH=$gatkPath,VCFMERGER=$VCFMERGER,VCFMERGER_DIR=$VCFMERGER_DIR,VCFSORTER=$VCFSORTER,RNA_VCF_HEADER=$RNA_VCF_HEADER,POST_MERGE_VENN=$POST_MERGE_VENN,DBSNP=$dbsnp,DBVERSION=$snpeffdb,HCVCF=$hcVcf,MPVCF=$mpVcf,FBVCF=$fbVcf,MERGERDIR=$mergerDir,ASSAYID=$assayID,BEDFILE=$bedFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_germVcfMerge.sh
                if [ $? -eq 0 ] ; then
