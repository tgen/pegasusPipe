	...
	if [ ! -e $fbTrackName.freebayes_All.vcf.snpEffPass  ] ; then
		...
		if [[ -e ${mergerDir}/${usableName}.vcfMergerPass || -e ${mergerDir}/${usableName}.vcfMergerInQueue || -e ${mergerDir}/${usableName}.vcfMergerFail ]] ; then
			...
			continue
                ...
                echo "qsub -A $debit -l nodes=1:ppn=8 -v VTPATH=$vtPath,SNPEFFPATH=$snpeffPath,SNPSIFT=$snpSift,DBNSFP=$DBNSFP,DBNSP=$DBNSP,TRACKNAME=$trackName,SAMTOOLS=$samTools,VARSCAN=$varScan,REF=$ref,DICT=$refDict,COSMIC=$COSMIC,KG=$KG,NHLBI=$NHLBI,SNPS=$snps,INDELS=$indels,GATKPATH=$gatkPath,VCFMERGER=$VCFMERGER,VCFMERGER_DIR=$VCFMERGER_DIR,VCFSORTER=$VCFSORTER,RNA_VCF_HEADER=$RNA_VCF_HEADER,POST_MERGE_VENN=$POST_MERGE_VENN,DBSNP=$dbsnp,DBVERSION=$snpeffdb,HCVCF=$hcVcf,MPVCF=$mpVcf,FBVCF=$fbVcf,MERGERDIR=$mergerDir,ASSAYID=$assayID,BEDFILE=$bedFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/pegasus_germVcfMerge.pbs"
                qsub -A $debit -l nodes=1:ppn=8 -v VTPATH=$vtPath,SNPEFFPATH=$snpeffPath,SNPSIFT=$snpSift,DBNSFP=$DBNSFP,DBNSP=$DBNSP,TRACKNAME=$trackName,SAMTOOLS=$samTools,VARSCAN=$varScan,REF=$ref,DICT=$refDict,COSMIC=$COSMIC,KG=$KG,NHLBI=$NHLBI,SNPS=$snps,INDELS=$indels,GATKPATH=$gatkPath,VCFMERGER=$VCFMERGER,VCFMERGER_DIR=$VCFMERGER_DIR,VCFSORTER=$VCFSORTER,RNA_VCF_HEADER=$RNA_VCF_HEADER,POST_MERGE_VENN=$POST_MERGE_VENN,DBSNP=$dbsnp,DBVERSION=$snpeffdb,HCVCF=$hcVcf,MPVCF=$mpVcf,FBVCF=$fbVcf,MERGERDIR=$mergerDir,ASSAYID=$assayID,BEDFILE=$bedFile,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pbsHome/pegasus_germVcfMerge.pbs
                if [ $? -eq 0 ] ; then
