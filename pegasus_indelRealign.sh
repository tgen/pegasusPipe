            ...
            if [ $arrayCount -gt 0 ] ; then
                ...
                if [ $skipGroup -ne 1 ] ; then
                    ...
                    do
                        ...
                        sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export GATKPATH=$gatkPath,INTS=$irIntFile,IRBAMFILE=$irBamFile,D=$d,INDELS=$indels,REF=$ref,BAMFILE=$bamName,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_indelRealign.sh
                        if [ $? -eq 0 ] ; then
