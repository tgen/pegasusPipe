...
VCFMERGER="/home/tgenref/binaries/vcfMerger/vcfMerger_V2_norm/production.version.VCFMERGER_V2.WithNorm.201600819/pecan.merge.3vcfs.main.sh"
VCFMERGER_DIR="/home/tgenref/binaries/vcfMerger/vcfMerger_V2_norm/production.version.VCFMERGER_V2.WithNorm.201600819"
...
POST_MERGE_VENN="/home/tgenref/binaries/vcfMerger/vcfMerger_V2_norm/production.version.VCFMERGER_V2.WithNorm.201600819/pecan.Venn_postMMRF_specific_filtering.sh"
KG="/home/tgenref/binaries/vcfMerger/vcfMerger/1000G_phase1.snps.high_confidence.b37.vcf"
COSMIC="/home/tgenref/binaries/vcfMergervcfMerger/CosmicCodingMuts_v66_20130725_sorted.vcf"
...
do
    ...
    if [ -z "$alleleCount" ] ; then
        ...
        if [ ! -e $mergerDir/$usableName.vcfMergerPass ] ; then
            ...
            sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,SNPEFFPATH=$snpeffPath,SNPSIFT=$snpSift,DBNSFP=$DBNSFP,SAMTOOLS=$samTools,VARSCAN=$varScan,REF=$ref,DICT=$refDict,COSMIC=$COSMIC,KG=$KG,NHLBI=$NHLBI,SNPS=$snps,INDELS=$indels,GATK=$gatkPath,VCFMERGER=$VCFMERGER,BASENAME=$usableName,VCFMERGER_DIR=$VCFMERGER_DIR,VCFSORTER=$VCFSORTER,RNA_VCF_HEADER=$RNA_VCF_HEADER,POST_MERGE_VENN=$POST_MERGE_VENN,DBSNP=$dbsnp,DBVERSION=$snpeffdb,MERGERDIR=$mergerDir,RNABAM=$rnaBam,ASSAYID=$assayID,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_mergeVcfAlleleCount.sh
            if [ $? -eq 0 ] ; then
