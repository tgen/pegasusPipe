...
do
	...
	sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$trackName.seurat.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
	if [ $? -eq 0 ] ; then
...
do
	...
	sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$trackName.REVseurat.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
	if [ $? -eq 0 ] ; then
...
do
	...
	if [[ -e $vcfPre.strelka.passed.somatic.snvs.vcf.snpEffInQueue || -e $vcfPre.strelka.passed.somatic.snvs.vcf.snpEffPass || -e $vcfPre.strelka.passed.somatic.snvs.vcf.snpEffFail ]] ; then 
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.passed.somatic.snvs.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
		if [ $? -eq 0 ] ; then
	...
	if [[ -e $vcfPre.strelka.all.somatic.snvs.vcf.snpEffInQueue || -e $vcfPre.strelka.all.somatic.snvs.vcf.snpEffPass || -e $vcfPre.strelka.all.somatic.snvs.vcf.snpEffFail ]] ; then 
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.all.somatic.snvs.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
		if [ $? -eq 0 ] ; then
	...
	if [[ -e $vcfPre.strelka.passed.somatic.indels.vcf.snpEffInQueue || -e $vcfPre.strelka.passed.somatic.indels.vcf.snpEffPass || -e $vcfPre.strelka.passed.somatic.indels.vcf.snpEffFail ]] ; then 
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.passed.somatic.indels.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
		if [ $? -eq 0 ] ; then
	...
	if [[ -e $vcfPre.strelka.all.somatic.indels.vcf.snpEffInQueue || -e $vcfPre.strelka.all.somatic.indels.vcf.snpEffPass || -e $vcfPre.strelka.all.somatic.indels.vcf.snpEffFail ]] ; then 
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$vcfPre.strelka.all.somatic.indels.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
		if [ $? -eq 0 ] ; then
...
do
	...
	sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
	if [ $? -eq 0 ] ; then
...
do
	...
	sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$trackName.HC_All.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
	if [ $? -eq 0 ] ; then
...
do
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$trackName.mpileup_All.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
        if [ $? -eq 0 ] ; then
...
do
        ...
        sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$trackName.freebayes_All.vcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
        if [ $? -eq 0 ] ; then
		...
		if [ $jrRequested -gt 0 ] ; then
			...
			if [[ ! -e $hcPas || ! -e $hcVcf ]] ; then
				...
				if [[ -e $hcVcf.snpEffPass || -e $hcVcf.snpEffInQueue || -e $hcVcf.snpEffFail ]] ; then
					...
					sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$hcVcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
					if [ $? -eq 0 ] ; then
			...
			if [[ ! -e $mpPas || ! -e $mpVcf ]] ; then
                                ...
                                if [[ -e $mpVcf.snpEffPass || -e $mpVcf.snpEffInQueue || -e $mpVcf.snpEffFail ]] ; then
                                        ...
                                        sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$mpVcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
                                        if [ $? -eq 0 ] ; then
			...
			if [[ ! -e $fbPas || ! -e $fbVcf ]] ; then
                                ...
                                if [[ -e $fbVcf.snpEffPass || -e $fbVcf.snpEffInQueue || -e $fbVcf.snpEffFail ]] ; then
                                        ...
                                        sbatch -n 1 -N 1 --cpus-per-task $nCores --export SNPEFFPATH=$snpeffPath,VCF=$fbVcf,DBSNP=$dbsnp,DBVERSION=$snpeffdb,RUNDIR=$runDir,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_snpEff.sh
                                        if [ $? -eq 0 ] ; then
