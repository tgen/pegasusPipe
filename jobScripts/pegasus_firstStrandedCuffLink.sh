...
if [ ${USEMASK} == "no" ] ; then
        ...
        perf stat ${CUFFLINKSPATH}/cufflinks ${PARAMS} --library-type fr-secondstrand --frag-bias-correct ${REF} --GTF ${CUFFLINKGTF} ${BAM} 2> ${DIRNAME}.cuffLink.perfOut > ${DIRNAME}.cuffLinkOut 2>&1
        if [ $? -eq 0 ] ; then
	...
	perf stat ${CUFFLINKSPATH}/cufflinks ${PARAMS} --library-type fr-secondstrand --frag-bias-correct ${REF} --GTF ${CUFFLINKGTF} -M ${CUFFLINKMASK} ${BAM} 2> ${DIRNAME}.cuffLink.perfOut > ${DIRNAME}.cuffLinkOut 2>&1
	if [ $? -eq 0 ] ; then
		newName=`basename ${BAM}`
		newName=${newName/.proj.Aligned.out.sorted.md.bam}
		mv ${DIRNAME}.cuffLinkOut ${DIRNAME}.cuffLinkPass
