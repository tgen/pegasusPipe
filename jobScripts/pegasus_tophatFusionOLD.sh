...
perf stat ${THFUSIONPATH}/tophat2 \
	...
	-o ${DIR} \
	${INDEXBASE} ${FASTQ1}.60mil.gz ${FASTQ2}.60mil.gz > ${DIR}.thFusionOut 2> ${DIR}.thFusion.perfOut
if [ $? -eq 0 ] ; then
	echo "success."
	echo "renaming..."
...
#rm -f ${FASTQ1}.trimPass
#rm -f ${FASTQ1}.trimFail
#rm -f ${FASTQ2}.trimPass
#rm -f ${FASTQ2}.trimFail
...
#echo "### we need trimming first on ${FASTQ1}"
#${TRIMFASTQPATH}/DPtrimFastq.pl ${FASTQ1} 0 $pos1 | gzip > $trimmedRead1 &
#f1id=$! #throw in background capture pid

#echo "### we need trimming first on ${FASTQ2}"
#${TRIMFASTQPATH}/DPtrimFastq.pl ${FASTQ2} 0 $pos2 | gzip > $trimmedRead2
#if [ $? -eq 0 ] ; then
#	echo "### trim finished OK"
#	touch ${FASTQ2}.trimPass
#else
