...
deseqPath=`grep @@"$recipe"@@ $constants | grep @@DESEQPATH= | cut -d= -f2`
...
do
    ...
    tophat) echo "tophat case"
        ...
        #fi
        deSeqDir=$runDir/deSeq/$deSeqName
        if [ ! -d $deSeqDir ] ; then
            mkdir -p $deSeqDir
        fi
        if [[ -e $deSeqDir.deSeqPass || -e $deSeqDir.deSeqFail || -e $deSeqDir.deSeqInQueue ]] ; then
            echo "### DeSeq is already done, failed or inQueue"
            continue
        fi

        echo "### Submitting $normlList2-VS-$tumorList2 to queue for deSeq..."
        sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,DESEQPATH=$deseqPath,RUNDIR=$runDir,DIRNAME=$deSeqDir,GTF=$gtf,NORMLIST="$normlList",TUMORLIST="$tumorList",REF=$ref,NXT1=$nxtStep1,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_deSeq.sh
        if [ $? -eq 0 ] ; then
