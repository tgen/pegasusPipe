...
do
	...
	tophat) echo "tophat case"
		...
		qsub -A $debit -l nodes=1:ppn=$nCores -v DESEQPATH=$deseqPath,RUNDIR=$runDir,DIRNAME=$deSeqDir,GTF=$gtf,NORMLIST="'"$normlList"'",TUMORLIST="'"$tumorList"'",REF=$ref,NXT1=$nxtStep1,D=$d $pbsHome/pegasus_deSeq.pbs
		if [ $? -eq 0 ] ; then
