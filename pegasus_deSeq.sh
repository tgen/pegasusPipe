...
do
	...
	tophat) echo "tophat case"
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export DESEQPATH=$deseqPath,RUNDIR=$runDir,DIRNAME=$deSeqDir,GTF=$gtf,NORMLIST="'"$normlList"'",TUMORLIST="'"$tumorList"'",REF=$ref,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_deSeq.sh
		if [ $? -eq 0 ] ; then
