...
do
    ...
    tophat) echo "### Tophat case"
            ...
            sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export SAMTOOLSPATH=$samtoolsPath,PICARDPATH=$picardPath,SAM=$accHitsSam,BAM=$accHitsBam,GTF=$gtf,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_htSeq.sh
            if [ $? -eq 0 ] ; then
    ...
    star) echo "### Star case"
            ...
            echo "### Submitting $alignedSam to queue for HT Seq..."
             if [[ $rnaStrand == "FIRST" ]] ; then
                ...
                sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export SAMTOOLSPATH=$samtoolsPath,BAM=$alignedBam,SAM=$alignedSam,GTF=$gtf,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_strandedHtSeqForStar.sh
                if [ $? -eq 0 ] ; then
                ...
                sleep 2
                                ...
                                echo "##running stranded STAR case"
                                sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export SAMTOOLSPATH=$samtoolsPath,BAM=$alignedBam,SAM=$alignedSam,GTF=$gtf,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_revStrandedHtSeqForStar.sh
                                if [ $? -eq 0 ] ; then
                                        touch $alignedSam.htSeqInQueue
                                else
                                        ((qsubFails++))
                ...
                sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export SAMTOOLSPATH=$samtoolsPath,BAM=$alignedBam,SAM=$alignedSam,GTF=$gtf,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_htSeqForStar.sh
                if [ $? -eq 0 ] ; then
                    touch $alignedSam.htSeqInQueue
                else
                    ((qsubFails++))
