...
do
	...
	tophatfusion) echo "### Tophatfusion fusion detector"
		...
		if [ $rnaStrand == "FIRST" ] ; then
			...
			sbatch -n 1 -N 1 --cpus-per-task $nCores --export FAI=$faiFile,PICARDPATH=$picardPath,REFPRETOPHAT=$ref,BWAPATH=$bwaPath,BOWTIE1PATH=$bowtie1Path,INDEXBASE=$indexBase,TOPHAT2PATH=$tophat2Path,THFUSIONPATH=$thfPath,SAMTOOLSPATH=$samtoolsPath,REF=$ref,FASTQ1=$read1Name,FASTQ2=$read2Name,DIR=$ownDir,INDEXBASE=$indexbase,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d,PRE1=$pre1,POS1=$pos1,PRE2=$pre2,POS2=$pos2 $pegasusPbsHome/pegasus_firstStrandedtophatFusion.pbs
			if [ $? -eq 0 ] ; then
	...
	soapFuse) echo "### Soap Fuse fusion detector"
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export SAMPLE=$samName,SLFILE=$ownDir/soapFuse.sampleList,SPCONFIG=$spConfig,SOAPFUSEPATH=$soapFusePath,SAMTOOLSPATH=$samtoolsPath,REF=$ref,FASTQ1=$read1Name,FASTQ2=$read2Name,DIR=$ownDir,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pecan_soapFuse.pbs
		if [ $? -eq 0 ] ; then
