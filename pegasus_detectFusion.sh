...
do
	...
	soapFuse) echo "### Soap Fuse fusion detector"
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export SAMPLE=$samName,SLFILE=$ownDir/soapFuse.sampleList,SPCONFIG=$spConfig,SOAPFUSEPATH=$soapFusePath,SAMTOOLSPATH=$samtoolsPath,REF=$ref,FASTQ1=$read1Name,FASTQ2=$read2Name,DIR=$ownDir,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pecan_soapFuse.sh
		if [ $? -eq 0 ] ; then
