...
do
    ...
    if [ "$assayID" == "RNA" ] ; then
        ...
        if [ ! -e $rnaBam ] ; then
            ...
            echo "sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out --export ALL,GATKPATH=$gatkPath,OUTBAM=$outBam,RNABAM=$rnaBam,REF=$ref,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_splitNCigarReads.sh"
