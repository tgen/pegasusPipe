...
do
    ...
    shortName=`cat $configFile | grep SHORTNAME=$sampleNames | cut -d= -f3`
    if [ ! -z "$shortName" ] ; then
        echo "### Short name detected: $shortName"
        usableName=$shortName
    fi
    sampleCount=0
    ...
    do
        ...
        eachSampleBam=$runDir/$kitName/$samName/$samName.proj.md.rr.bam
        eachSamplePass=$runDir/$kitName/$samName/$samName.proj.md.bam.rrPass
        if [[ ! -e $eachSampleBam || ! -e $eachSamplePass ]] ; then
            echo "### Can't find the rr bam or the jr pass"
    ...
    mkdir -p $rbDir/$usableName
    workDir="$rbDir/$usableName"
    trackName="$runDir/rb/$usableName/$usableName"
    if [[ -e $trackName.ugInQueue || -e $trackName.ugPass || -e $trackName.ugFail ]] ; then
        echo "### Joint Uni Gen on Reduced Bams is already done, failed or inQueue"
        continue
    fi
    echo "### Submitting $usableName to queue for joint Uni Gen on Reduced Bams..."
    sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,GATKPATH=$gatkPath,TRK=$trackName,KNOWN=$snps,BAMLIST="$sampleList",REF=$ref,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_unifiedGenotyper.sh
    if [ $? -eq 0 ] ; then
...
#                    sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,GATKPATH=$gatkPath,TRK=$mdBam,KNOWN=$snps,BAMLIST=$mdBam,REF=$ref,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_unifiedGenotyperSingle.sh
#                    if [ $? -eq 0 ] ; then
