					...
					if [ $missingFastqSample -eq 0 ] ; then
							...
							echo "### submitting $ownDir to queue for Kallisto... "							
							 ...
							 if [[ $rnaStrand == "FIRST" ]] ; then
								...
								qsub -A $debit -l nodes=1:ppn=$nCores -v KALLISTO_INDEX_CDNA=$kallistoIndexCDNA,KALLISTO_INDEX_GTF=$kallistoIndexGTF,SAMNAME=$samName,FASTQL="'"$fastqList"'",DIR=$ownDir,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_firstStrandedKallisto.pbs
								if [ $? -eq 0 ] ; then
								...
								qsub -A $debit -l nodes=1:ppn=$nCores -v KALLISTO_INDEX_CDNA=$kallistoIndexCDNA,KALLISTO_INDEX_GTF=$kallistoIndexGTF,SAMNAME=$samName,FASTQL="'"$fastqList"'",DIR=$ownDir,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_secondStrandedKallisto.pbs
                                                                if [ $? -eq 0 ] ; then
                                                                        touch $ownDir.kallistoInQueue
                                                                else
                                                                        ((qsubFails++))
								...
								qsub -A $debit -l nodes=1:ppn=$nCores -v KALLISTO_INDEX_CDNA=$kallistoIndexCDNA,KALLISTO_INDEX_GTF=$kallistoIndexGTF,SAMNAME=$samName,FASTQL="'"$fastqList"'",DIR=$ownDir,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_kallisto.pbs
                                                                if [ $? -eq 0 ] ; then
                                                                        touch $ownDir.kallistoInQueue
                                                                else
                                                                        ((qsubFails++))
