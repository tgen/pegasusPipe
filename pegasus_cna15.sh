		...
		GenomeGenome) echo "### Both normal and tumor are Genome"
				...
				if [ "$cnaGenomeUnFilter" = "yes" ] ; then
					...
					if [[ -e $trackName.cnaGenUnfiInQueue || -e $trackName.cnaGenUnfiPass || -e $trackName.cnaGenUnfiFail ]] ; then
						...
						echo "### Submitting to queue with $trackName"
						sbatch -n 1 -N 1 --cpus-per-task $nCores -v TRACKNAME=$trackName,MYPATH=$cnaDir/$outName2,CCDSLIST=$ccdsPath,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair1SamName,TUMORSAMPLE=$pair2SamName,NORMALDAT=$normalDatFile,TUMORDAT=$tumorDatFile,OFILE=$outName2,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d $pbsHome/pegasus_cnaGenUnfi15.pbs
						if [ $? -eq 0 ] ; then
		...
		do
			...
			ExomeExome) echo "### Both normal and tumor are Exome"
					...
					if [[ -e $trackName.cnaExomeInQueue || -e $trackName.cnaExomePass || -e $trackName.cnaExomeFail ]] ; then
						...
						echo "### Submitting to queue with $trackName"
						sbatch -n 1 -N 1 --cpus-per-task $nCores -v NHETDEPTH=$nHetDepth1,THETDEPTH=$tHetDepth,TRACKNAME=$trackName,CCDSLIST=$ccdsPath,MYPATH=$cnaDir/$outNameDad,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair2SamName,TUMORSAMPLE=$pair1SamName,CNAEXOMETARGET=$bedFile,NORMALDAT=$normalDatFile1,TUMORDAT=$tumorDatFile,OFILE=$outNameDad,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d $pbsHome/pegasus_cnaExo15.pbs
						if [ $? -eq 0 ] ; then
							touch $trackName.cnaExomeInQueue
							sleep 1
						else
					...
					if [[ -e $trackName.cnaExomeInQueue || -e $trackName.cnaExomePass || -e $trackName.cnaExomeFail ]] ; then
						...
						echo "### Submitting to queue with $trackName"
						sbatch -n 1 -N 1 --cpus-per-task $nCores -v NHETDEPTH=$nHetDepth2,THETDEPTH=$tHetDepth,TRACKNAME=$trackName,CCDSLIST=$ccdsPath,MYPATH=$cnaDir/$outNameMom,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair3SamName,TUMORSAMPLE=$pair1SamName,CNAEXOMETARGET=$bedFile,NORMALDAT=$normalDatFile2,TUMORDAT=$tumorDatFile,OFILE=$outNameMom,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d $pbsHome/pegasus_cnaExo15.pbs
						if [ $? -eq 0 ] ; then
							touch $trackName.cnaExomeInQueue
							sleep 1
						else
