    ...
    if [ $famORpair == "DNAPAIR" ] ; then
        ...
        if [[ ${matchedNormal} == "false" ]] ; then
            ...
            if [[ "$pair1KitName" == *S5U ]]  ; then
                ...
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/hs37d5_tgen/tool_specific_resources/tconut/agilent_sureselect_v6r2_strexomeProstateV2/Agilent_SureSelect_V6R2_StxProstate_hs37d5_Padded_DGV_1kg.cna.bed"
            elif [[ "$pair1KitName" == *ST2 ]]  ; then
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/hs37d5_tgen/tool_specific_resources/tconut/agilent_sureselect_v6r2_strexomeV2/Agilent_SureSelect_V6R2_StrexomeV2_hs37d5_Padded_DGV_1kg.cna.bed"
            elif [[ "$pair1KitName" == *S2U ]]  ; then
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/hs37d5_tgen/tool_specific_resources/tconut/agilent_sureselect_v2_NA/Agilent_SureSelect_V2_NA_hs37d5_Padded_DGV_1kg.cna.bed"
            elif [[ "$pair1KitName" == *SHK ]] ; then
                ...
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/agilent_sureselect_all_exome_v7/agilent_sureselect_all_exome_v7_regions_preprocessed.unmatched_cna.bed"
            elif [[ "$pair1KitName" == *TSC ]] ; then
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/agilent_custom_tsc_NA/agilent_custom_tsc_NA.unmatched_cna.bed"
            elif [[ "$pair1KitName" == *GEX ]] ; then
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/generic_custom_grch37_humanexome/grch37_all_exons_1_to_xy.unmatched_cna.bed"
            elif [[ "$pair1KitName" == *PTC ]] ; then
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/agilent_custom_johnchan2017_v1/John_Chan_2017_new_1_Covered.ens.unmatched_cna.bed"
            elif [[ "$pair1KitName" == *NEZ ]] ; then
            ...
            elif [[ "$pair1KitName" == *ICG ]] ; then
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/agilent_sureselect_ICGC50Mb/SureSelect_All_Exon_50mb_hg19a.pre.unmatched_cna.bed"
            elif [[ "$pair1KitName" == *S5C ]] ; then
                bedFile="/home/tgenref/homo_sapiens/grch37_hg19/capture_targets/agilent_sureselect_v5_colo829/SureSelectV5_UTR_COLO.unmatched_cna.bed"
            else
        ...
        GenomeGenome) echo "### Both normal and tumor are Genome"
                ...
                if [ "$cnaGenomeFilter" = "yes" ] ; then
                    ...
                    if [[ -e $trackName.cnaGenFiltInQueue || -e $trackName.cnaGenFiltPass || -e $trackName.cnaGenFiltFail ]] ; then
                        ...
                        sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,TRACKNAME=$trackName,MYPATH=$cnaDir/$outName1,CCDSLIST=$ccdsPath,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair1SamName,TUMORSAMPLE=$pair2SamName,NORMALDAT=$normalDatFile,TUMORDAT=$tumorDatFile,OFILE=$outName1,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_cnaGenFilt15.sh
                        if [ $? -eq 0 ] ; then
                ...
                if [ "$cnaGenomeUnFilter" = "yes" ] ; then
                    ...
                    if [[ -e $trackName.cnaGenUnfiInQueue || -e $trackName.cnaGenUnfiPass || -e $trackName.cnaGenUnfiFail ]] ; then
                        ...
                        echo "### Submitting to queue with $trackName"
                        sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,TRACKNAME=$trackName,MYPATH=$cnaDir/$outName2,CCDSLIST=$ccdsPath,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair1SamName,TUMORSAMPLE=$pair2SamName,NORMALDAT=$normalDatFile,TUMORDAT=$tumorDatFile,OFILE=$outName2,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_cnaGenUnfi15.sh
                        if [ $? -eq 0 ] ; then
        ...
        ExomeExome) echo "### Both normal and tumor are Exome"
                ...
                if [[ -e $trackName.cnaExomeInQueue || -e $trackName.cnaExomePass || -e $trackName.cnaExomeFail ]] ; then
                    ...
                    echo "### Submitting to queue with $trackName"
                    sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,NHETDEPTH=$nHetDepth,THETDEPTH=$tHetDepth,TRACKNAME=$trackName,CCDSLIST=$ccdsPath,MYPATH=$cnaDir/$outName3,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair1SamName,TUMORSAMPLE=$pair2SamName,CNAEXOMETARGET=$bedFile,NORMALDAT=$normalDatFile,TUMORDAT=$tumorDatFile,OFILE=$outName3,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_cnaExo15.sh
                    if [ $? -eq 0 ] ; then
        ...
        do
            ...
            ExomeExome) echo "### Both normal and tumor are Exome"
                    ...
                    if [[ -e $trackName.cnaExomeInQueue || -e $trackName.cnaExomePass || -e $trackName.cnaExomeFail ]] ; then
                        ...
                        echo "### Submitting to queue with $trackName"
                        sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,NHETDEPTH=$nHetDepth1,THETDEPTH=$tHetDepth,TRACKNAME=$trackName,CCDSLIST=$ccdsPath,MYPATH=$cnaDir/$outNameDad,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair2SamName,TUMORSAMPLE=$pair1SamName,CNAEXOMETARGET=$bedFile,NORMALDAT=$normalDatFile1,TUMORDAT=$tumorDatFile,OFILE=$outNameDad,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_cnaExo15.sh
                        if [ $? -eq 0 ] ; then
                            touch $trackName.cnaExomeInQueue
                            sleep 1
                        else
                    ...
                    if [[ -e $trackName.cnaExomeInQueue || -e $trackName.cnaExomePass || -e $trackName.cnaExomeFail ]] ; then
                        ...
                        echo "### Submitting to queue with $trackName"
                        sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,NHETDEPTH=$nHetDepth2,THETDEPTH=$tHetDepth,TRACKNAME=$trackName,CCDSLIST=$ccdsPath,MYPATH=$cnaDir/$outNameMom,CNAPATH=$cnaPath,MERGEDVCF=$vcfFile,NORMALSAMPLE=$pair3SamName,TUMORSAMPLE=$pair1SamName,CNAEXOMETARGET=$bedFile,NORMALDAT=$normalDatFile2,TUMORDAT=$tumorDatFile,OFILE=$outNameMom,ASSAY=$assay,GTF=$gtf,NXT1=$nxtStep1,REF=$ref,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_cnaExo15.sh
                        if [ $? -eq 0 ] ; then
                            touch $trackName.cnaExomeInQueue
                            sleep 1
                        else
