...
do
	...
	tophat) echo "tophat case"
		...
		qsub -A $debit -l nodes=1:ppn=$nCores -v PARAMS=${params},DIRNAME=$topHatDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$accHitsBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_cuffQuant.pbs
		if [ $? -eq 0 ] ; then
	...
	star) echo "star case"
		...
		if [ $rnaStrand == "FIRST" ] ; then
			...
			qsub -A $debit -l nodes=1:ppn=$nCores -v PARAMS=$params,DIRNAME=$starDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$starBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_firstStrandedCuffQuant.pbs
			if [ $? -eq 0 ] ; then
			...
			sleep 2
                        ...
                        qsub -A $debit -l nodes=1:ppn=$nCores -v PARAMS=$params,DIRNAME=$starDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$starBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_secondStrandedCuffQuant.pbs
                        if [ $? -eq 0 ] ; then
                                touch $starDir.cuffQuantInQueue
                        else
                                ((qsubFails++))
			...
			qsub -A $debit -l nodes=1:ppn=$nCores -v PARAMS=$params,DIRNAME=$starDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$starBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pbsHome/pegasus_cuffQuant.pbs
			if [ $? -eq 0 ] ; then
				touch $starDir.cuffQuantInQueue
			else
				((qsubFails++))
