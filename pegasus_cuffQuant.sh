...
do
	...
	tophat) echo "tophat case"
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export PARAMS=${params},DIRNAME=$topHatDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$accHitsBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_cuffQuant.pbs
		if [ $? -eq 0 ] ; then
	...
	star) echo "star case"
		...
		if [ $rnaStrand == "FIRST" ] ; then
			...
			sbatch -n 1 -N 1 --cpus-per-task $nCores --export PARAMS=$params,DIRNAME=$starDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$starBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_firstStrandedCuffQuant.pbs
			if [ $? -eq 0 ] ; then
			...
			sleep 2
                        ...
                        sbatch -n 1 -N 1 --cpus-per-task $nCores --export PARAMS=$params,DIRNAME=$starDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$starBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_secondStrandedCuffQuant.pbs
                        if [ $? -eq 0 ] ; then
                                touch $starDir.cuffQuantInQueue
                        else
                                ((qsubFails++))
			...
			sbatch -n 1 -N 1 --cpus-per-task $nCores --export PARAMS=$params,DIRNAME=$starDir,CUFFQUANTPATH=$cuffQuantPath,REF=$ref,BAM=$starBam,USEGTF=$usegtf,USEMASK=$usemask,CUFFLINKGTF=$gtf,CUFFLINKMASK=$gtfmask,NXT1=$nxtStep1,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_cuffQuant.pbs
			if [ $? -eq 0 ] ; then
				touch $starDir.cuffQuantInQueue
			else
				((qsubFails++))
