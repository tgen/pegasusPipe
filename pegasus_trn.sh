...
do
	...
	*)	echo "### Something else"
	...
	sbatch -n 1 -N 1 --cpus-per-task $nCores --export ASSAY=$assay,BEDFILE=$bedFile,GTF=$gtf,NORMAL=$normalBamFile,TUMOR=$tumorBamFile,OUTFILE=$trnDatFile,TRNPATH=$trnPath,SAMPATH=$samtoolsPath,RUNDIR=$runDir,RECIPE=$recipe,NXT1=$nxtStep1,D=$d $pegasusPbsHome/pegasus_trn.pbs
	if [ $? -eq 0 ] ; then
