                ...
                if [ $skipGroup -ne 1 ] ; then
                    ...
                    if [ $missingFastqSample -eq 0 ] ; then
                            ...
                            if [[ $rnaStrand == "FIRST" || $rnaStrand == "SECOND" ]] ; then
                                ...
                                sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --mem-per-cpu 6000 --export ALL,RGTAGLIST="$rgTagList",SAMNAME=$samName,SAMTOOLSPATH=$samtoolsPath,STARPATH=$starPath,STARREF=$starRef,STARGTF=$starGTF,FASTQL1="$fastqList1",FASTQL2="$fastqList2",DIR=$ownDir,NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,NXT4=$nxtStep4,NXT5=$nxtStep5,NXT6=$nxtStep6,NXT7=$nxtStep7,NXT8=$nxtStep8,NXT9=$nxtStep9,NXT10=$nxtStep10,NXT11=$nxtStep11,RUNDIR=$runDir,RNASTRAND=$rnaStrand,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_strandedStar.sh
                                if [ $? -eq 0 ] ; then
