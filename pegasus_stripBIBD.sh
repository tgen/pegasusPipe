...
thisStep="pegasus_nextJob_stripBIBD.txt"
...
do
	...
	echo "### Submitting to queue to run hc on $wd"
		if [[ -e ${trackName}.stripBIBDInQueue || -e ${trackName}.stripBIBDPass || -e ${trackName}.stripBIBDFail ]] ; then
			echo "### strip BI/BD is already done, failed, or inqueue for ${trackName}"
			continue
		fi

			echo Starting strip BI/BD Step${STEP}
			qsub -A $debit -l nodes=1:ppn=$nCores -v GATKPATH=$gatkPath,STEPCOUNT=$STEP_COUNT,TRK=$trackName,KNOWN=$snps,BAMLIST="'$sampleList'",TRK=$trackName,CHRLIST=$chrList,REF=$ref,STEP=${STEP},NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,RUNDIR=$runDir,D=$d $pbsHome/pegasus_haplotypeCaller.pbs
			if [ $? -eq 0 ] ; then
				touch ${trackName}.stripBIBDInQueue
			else
				((qsubFails++))
			fi
		#fi
...
do
	...
	if [ "$assayID" != "RNA" ] ; then
		...
		if [ $jrRequested -gt 0 ] ; then
			echo "### strip BI/BD will not operate on a single bam because Joint IR is requested for $samName"
		else
			echo "### strip BI/BD will run on single bam because Joint IR is NOT requested for $samName"
			if [[ ! -e $inBam.mdPass || ! -e $mdBam ]] ; then
				...
				do
					(( STEP++ ))
					if [[ -e ${mdBam}.stripBIBDInQueue || -e ${mdBam}.stripBIBDPass || -e ${mdBam}.stripBIBDFail ]] ; then
						echo "### strip BI/BD is already done, failed, or inqueue for ${mdBam}"
						continue
					fi

					echo Starting strip BI/BD for Step${STEP}
					qsub -A $debit -l nodes=1:ppn=$nCores -v GATKPATH=$gatkPath,STEPCOUNT=$STEP_COUNT,TRK=$mdBam,KNOWN=$snps,BAMLIST=$mdBam,CHRLIST=$chrList,REF=$ref,STEP=${STEP},NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,RUNDIR=$runDir,D=$d $pbsHome/pegasus_haplotypeCallerSingle.pbs
					if [ $? -eq 0 ] ; then
						touch ${mdBam}.stripBIBDInQueue
					else
						((qsubFails++))
					fi
