...
do
	...
	if [ $missingGrp -eq 0 ] ; then
		...
		qsub -A $debit -l nodes=1:ppn=$nCores -v FILELIST="$fileList",NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,MERGEDVCF=$mergedVCF,RUNDIR=$runDir,D=$d $pbsHome/pegasus_mergeVCFs.pbs
		if [ $? -eq 0 ] ; then
