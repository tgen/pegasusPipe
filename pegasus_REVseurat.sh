...
revSeurat=`grep @@"$recipe"@@ $constants | grep @@REVSEURAT= | cut -d= -f2`
...
do
    if [ "$revSeurat" == "no" ] ; then
            echo "### reverse seurat is $revSeurat. Will skip."
            continue
    fi
    ...
    do
        (( STEP++ ))
        if [[ -e ${trackName}_Step${STEP}.REVseuratInQueue || -e ${trackName}_Step${STEP}.REVseuratPass || -e ${trackName}_Step${STEP}.REVseuratFail ]] ; then
            echo "### Seurat caller is already done, failed, or inqueue for ${trackName}"
            continue
        fi

        echo Starting Seurat caller Step${STEP}
        sbatch --output $runDir/oeFiles/%x-slurm-%j.out --export STEPCOUNT=$STEP_COUNT,GATKPATH=$gatkPath,SEURATPATH=$seuratPath,NXT1=$nxtStep1,NXT2=$nxtStep2,TRK=$trackName,CHRLIST=$chrList,REF=$ref,STEP=${STEP},NORMAL=$normalBamFile,TUMOR=$tumorBamFile,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_REVseurat.sh
        if [ $? -eq 0 ] ; then
