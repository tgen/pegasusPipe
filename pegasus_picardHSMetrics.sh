    ...
    if [ "$assayID" == "Exome" ] ; then
        ...
        if [[ ! -e $inBam.mdPass || ! -e $mdBam ]] ; then
            ...
            if [[ -e $mdBam.picHSMetricsPass || -e $mdBam.picHSMetricsInQueue || -e $mdBam.picHSMetricsFail ]] ; then
                ...
                sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,BAITS=$baits,TARGETS=$targets,PICARDPATH=$picardPath,RUNDIR=$runDir,REF=$ref,BAMFILE=$mdBam,DIR=$pcDir,NXT1=$nxtStep1,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_picardHSMetrics.sh
                if [ $? -eq 0 ] ; then
        ...
        if [ $jrRequested -gt 0 ] && [ $jirRequested != "no" ] ; then
            ...
            if [[ ! -e $jrBam.jointIRPass || ! -e $jrBam ]] ; then
                ...
                if [[ -e $jrBam.picHSMetricsPass || -e $jrBam.picHSMetricsInQueue || -e $jrBam.picHSMetricsFail ]] ; then
                    ...
                    sbatch --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,BAITS=$baits,TARGETS=$targets,PICARDPATH=$picardPath,RUNDIR=$runDir,REF=$ref,BAMFILE=$jrBam,DIR=$pcDir,NXT1=$nxtStep1,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_picardHSMetrics.sh
                    if [ $? -eq 0 ] ; then
