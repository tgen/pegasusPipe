...
do
	...
	qsub -A $debit -l nodes=1:ppn=$nCores -v GATKPATH=$gatkPath,REF=$ref,BAMFILE=$mdBamFile,NXT1=$nxtStep1,RUNDIR=$runDir,OUTPUTBAM=$rrBamFile,D=$d $pbsHome/pegasus_gatkReduceReads.pbs
	if [ $? -eq 0 ] ; then
