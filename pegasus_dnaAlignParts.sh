					...
					do
						...
						do
							...
							bwamem) echo "### Submitting to qbwaMem to create $bamMiniName"
								...
								echo "$rgTag"
								sbatch -n 1 -N 1 --cpus-per-task $nCores --export D=$d,RGTAG=$rgTag,FASTQ1=$thisMiniFq,FASTQ2=$thisMiniR2,REF=$ref,BWAPATH=$bwaPath,SAMTOOLSPATH=$samtoolsPath,FAI=$faiFile,BAMPRE=$bamMiniPre,RUNDIR=$runDir,NXT1=$nxtStep1,NXT2=$nxtStep2,D=$d $pegasusPbsHome/pegasus_bwaMem.sh
								if [ $? -eq 0 ] ; then
