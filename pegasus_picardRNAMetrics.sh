...
do
    ...
    *) echo "### I should not be here"
    ...
    if [ $rnaStrand == "FIRST" ] ; then
        ...
        sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,REF=$ref,REFFLAT=$refFlat,RIBINTS=$ribInts,PICARDPATH=$picardPath,BAMFILE=$rnaBam,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_FSpicardRNAMetrics.sh
        if [ $? -eq 0 ] ; then
        ...
        sleep 2
                ...
                echo "##running stranded picard metrics case"
                sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,REF=$ref,REFFLAT=$refFlat,RIBINTS=$ribInts,PICARDPATH=$picardPath,BAMFILE=$rnaBam,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_SSpicardRNAMetrics.sh
                if [ $? -eq 0 ] ; then
                        touch $rnaBam.picRNAMetricsInQueue
                else
                        ((qsubFails++))
        ...
        sbatch --account ${debit} --output $runDir/oeFiles/%x-slurm-%j.out -n 1 -N 1 --cpus-per-task $nCores --export ALL,REF=$ref,REFFLAT=$refFlat,RIBINTS=$ribInts,PICARDPATH=$picardPath,BAMFILE=$rnaBam,RUNDIR=$runDir,D=$d ${JETSTREAM_HOME}/pegasusPipe/jobScripts/pegasus_picardRNAMetrics.sh
        if [ $? -eq 0 ] ; then
            touch $rnaBam.picRNAMetricsInQueue
        else
            ((qsubFails++))
