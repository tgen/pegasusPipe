...
do
	...
	do
		...
		sbatch -n 1 -N 1 --cpus-per-task $nCores --export STEPCOUNT=$STEP_COUNT,GATKPATH=$gatkPath,SEURATPATH=$seuratPath,NXT1=$nxtStep1,NXT2=$nxtStep2,NXT3=$nxtStep3,TRK=$trackName,CHRLIST=$chrList,REF=$ref,STEP=${STEP},NORMAL=$normalBamFile,TUMOR=$tumorBamFile,RUNDIR=$runDir,D=$d $pegasusPbsHome/pegasus_seurat.pbs
		if [ $? -eq 0 ] ; then
